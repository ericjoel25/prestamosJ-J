...
export default function RegistroDePago(props) {
  ...
  const [formData, setFormData] = useState({
    ...
    InicialDate:'',
    Intervalo:'',
    PaymentDate: Instant.RegularFormatHoy(date),
  ...
  return (
    ...
    <ScrollView style={styles.contener} >
      ...
      <LinearGradient colors={['#4B1248', '#FFA17F', '#4B1248']} >
        ...
        <View style={styles.InputContener}>
          <Text style={styles.TextInput}> Fecha De Pago:</Text>
          <TouchableOpacity>
            <Text style={[styles.Input, styles.TextDate]}
              onPress={showDatepicker} onChangeText={(value) => handleChangeTest('PaymentDate', value)} value={formData.PaymentDate} > {date
                ? Instant.RegularFormatHoy(date)
                : 'Fecha del pago'}
            </Text>
          </TouchableOpacity>
        </View>
